...
async function handleICal(request: Request): Promise<Response> {
  ...
  const { offset, limit, region, timezone, download } = extractCommonParams(params)
  ...
  const ical = generateCalendar(contests, lc.getContestUrl, offset, limit, timezone, request.url)
...
async function handleSvg(request: Request): Promise<Response> {
  ...
  const lc = new LCApi(region)
  const contests = await lc.fetchContests()
  const svg = generateSvg(contests, lc.getContestUrl, offset, limit, width, height, timezone, request.url)
...
function extractCommonParams(params: URLSearchParams): CommonParams {
  ...
  let download
  if ([null, undefined, "false", "0"].some((falsyValue) => params.get("download") === falsyValue)) {
    download = false
  }
  else {
    download = true
  ...
  const timezone = params.get('timezone') || (region === "CN" ? "Asia/Shanghai" : undefined)
  return {
    offset, limit, region, timezone, download
  }
}
